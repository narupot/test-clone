...
(function(){
	...
	angular.module('siam-app',['ct.ui.router.extras','jsonseivice','paginationDir'], function($interpolateProvider){
		$interpolateProvider.startSymbol('<%');
		$interpolateProvider.endSymbol('%>');
	});
	//config blok used for config al route related functionality
	angular.module('siam-app').config(['$stateProvider','$urlRouterProvider', '$locationProvider','$stickyStateProvider','$provide', '$compileProvider',function($stateProvider,$urlRouterProvider, $locationProvider,$stickyStateProvider,$provide, $compileProvider) {
	...
	angular.module('siam-app').filter('trustAsResourceUrl',function($sce) {
		return $sce.trustAsResourceUrl(val);
	});
	//This filter used for add dynamic JSON html in angular template
	angular.module('siam-app').filter('unsafe', function($sce) {
