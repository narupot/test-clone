jQuery(document).ready(function($){
    ...
    $('body').on('click','#add_user_address',function(){
        ...
        } else if (typeof create_address_url !== 'undefined') {
            $('#icon_loader').show();
            $('#popupdiv').load(create_address_url, {call_type:'ajax_data'}, function(){
                $('#icon_loader').hide();
            });
        }
    });
        ...
        if(country_isd_code == '66') {
            ...
            $(".autofill").autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    if ($('#sub_district').length) $('#sub_district').val(str_arr['2']);
...
function cleanCountryRelatedData() {
    $('#province_state').val('');
    $('#city_district').val('');
    if ($('#sub_district').length) $('#sub_district').val('');
} 