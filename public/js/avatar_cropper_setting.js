 ...
 console.log("here???");
    ...
    //Listen on button click on blog page after crope done
       ...
       if (!this.$avatarSrc.val() && !this.$avatarInput.val()) {
          ...
          var upload_status = $('#avatar_image').attr("data-image_upload_status");
          if(upload_status!== undefined && upload_status!="false"){
            ...
            $('#avatar_image').attr("data-image_upload_status", false);
            $("#avatar_image_input").val("");
            return false;
          }          
       }

       if (this.support.formData) {       
         $('#avatar_image').attr("src", croppedImage);
         $("#avatar_image_input").val(croppedImage);
         $('#avatar_image').attr("data-image_upload_status", true);
         this.cropDone("not_form");
         this.$avatarInput.val('');
         this.uploaded = true;
         return false;
    ...
    ajaxUpload: function () {
      ...
      $.ajax(url, {
        ...
        success: function (data) {
          _this.submitDone(data);
          $('#avatar_image').val(data.filename);        
        },
