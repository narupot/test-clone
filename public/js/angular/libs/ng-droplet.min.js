        ...
        return {
            ...
            controller: ['$scope', function DropletController($scope) {
                ...
                $scope.traverseFiles = function traverseFiles(files,prdId,prdIndex,optId,mainPrdId) {
                   //set file in custom droplet container
                   var temp_files = angular.extend({}, files);
                   if(count === 0){
                        $scope.interface.addRemoveCustomData("add_file", temp_files);                        
                   }
                   ...
                   for (var index = 0, numFiles = files.length; index < numFiles; index++) {
                        ...
                        if (!$scope.isValidExtension(extension) || current >= maximum) {
                            ...
                            //in case of product check two file are same or not if yes then set invalid
                            var res = $scope.interface.addRemoveCustomData("check_exist", current_droplet_file, file);
                            
                            if(res === "yes"){
                               type = $scope.FILE_TYPES.DELETED; 
                            }else{
                               current_droplet_file.push(file);
                            }
                        }
                ...
                (function setupDirectiveInterface() {
                    ...
                    $scope.interface = {
                        ...
                        *@param : action {string}
                        *@param : files_list {object}
                        *@param : f_item {object}
                        */
                        addRemoveCustomData : function(action, files_list, f_item){
                            var result = "no";

                            switch(action){
                                case "add_file" :                                    
                                    $.map(files_list, function(elem){                                       
                                        current_droplet_file.push(elem); 
                                    });
                                    break;
                                case "remove_file" :                                   
                                    var f_ind = current_droplet_file.findIndex(function(o){
                                        return (o.name === f_item.name && o.size === f_item.size);
                                    });
                                    
                                    if(f_ind!= -1){
                                       current_droplet_file.splice(f_ind, 1); 
                                    }
                                    
                                    break;
                                case "check_exist" :
                                    $.map(current_droplet_file, function(elem){                                                                                
                                        if(elem.name === f_item.name && elem.size === f_item.size){
                                            result = "yes";   
                                        }
                                    });
                                    break;
