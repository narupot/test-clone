;(function(){
	angular.module('smm-app').directive('getInputfieldDir', function() {
		...
		return {
			restict : 'E',
			template : '<select ng-model="viewItemPerPage" ng-options="page for page in gridOptions.paginationPageSizes" ng-change="HeaderPagination.pageSizeChange(viewItemPerPage)"></select> <span class="countPage">Per page</span> <span class="glyphicon glyphicon-menu-left" id="headerPrevBtn" ng-click="HeaderPagination.previousPage()"></span><input type="number" min="1" max="<%HeaderPagination.getTotalPages() %>" ng-model="gridOptions.paginationCurrentPage" ng-change="HeaderPagination.pageChange()">of <%HeaderPagination.getTotalPages()%><span class="glyphicon glyphicon-menu-right next" id="headerNextBtn" data="1" ng-click="HeaderPagination.nextPage()"> </span>'
		}
		...
		return {
			restict : 'E',
			template : '<select ng-model="vm.viewItemPerPage" ng-options="page for page in vm.gridOptions.paginationPageSizes" ng-change="vm.HeaderPagination.pageSizeChange(vm.viewItemPerPage)"></select> <span class="countPage">Per page</span> <span class="glyphicon glyphicon-menu-left" id="headerPrevBtn" ng-click="vm.HeaderPagination.previousPage()"></span><input type="number" min="1" max="<% vm.HeaderPagination.getTotalPages() %>" class="header-input-page" ng-model="vm.gridOptions.paginationCurrentPage" ng-change="vm.HeaderPagination.pageChange()">of <% vm.HeaderPagination.getTotalPages()%><span class="glyphicon glyphicon-menu-right next" id="headerNextBtn" data="1" ng-click="vm.HeaderPagination.nextPage()"> </span>'
		}
	})
