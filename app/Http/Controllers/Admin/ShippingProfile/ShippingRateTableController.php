...
use App\MasterGeography;
use App\MasterProvince;
use App\MasterDistrict;
use App\MasterSubDistrict;
use Auth;
use Lang;
use Config;
use File;
...
class ShippingRateTableController extends MarketPlace
{
    ...
    public function deliveryAtAddress(Request $request)
    {
        ...
        $allRegions = \DB::table('delivery_region as reg')
            ->select(
                'reg.reg_id',
                'reg.reg_name',
                'reg.status',
                'reg.reg_type',
                // ใช้ smm_adm ตามที่ Laravel ตั้งชื่อให้ใน SQL
                \DB::raw("CONCAT(smm_adm.first_name, ' ', smm_adm.last_name) as creator_name"),
                'adm.nick_name as creator_nickname',

                // ใส่ชื่อตารางเต็มๆ ใน Subquery ป้องกัน Prefix Missing
                \DB::raw('(SELECT COUNT(*) FROM smm_delivery_time_slot WHERE reg_id = smm_reg.reg_id) as slots_count'),

                // ใส่ชื่อตารางเต็มๆ ในส่วนการ Join พื้นที่
                \DB::raw('(SELECT GROUP_CONCAT(DISTINCT p.name_th SEPARATOR ", ")
            FROM smm_delivery_region_detail rd 
            JOIN smm_master_sub_districts sd ON rd.subdistrict_id = sd.id
            JOIN smm_master_districts d ON sd.district_id = d.id
            JOIN smm_master_provinces p ON d.province_id = p.id
            WHERE rd.region_id = smm_reg.reg_id) as area_summary')
            )
            ->leftJoin('admin_users as adm', 'reg.created_by', '=', 'adm.id')
            ->where('reg.st_del', 0)
            ->get();
    ...
    {
        $districts = MasterDistrict::where('province_id', $provinceId)
            ->orderBy('name_th', 'asc')
            ->get();
    ...
    public function getSubDistricts($districtId)
    {
        ...
        $subDistricts = MasterSubDistrict::where('district_id', $districtId)
            ->orderBy('name_th', 'asc')
            ->get();
        $subDistrictIds = $subDistricts->pluck('id')->toArray();
        $usedLookup = [];

        if (!empty($subDistrictIds)) {
            $usedData = DeliveryRegionDetail::whereIn('subdistrict_id', $subDistrictIds)
                ->where('status', 1)
                ->with('region')
                ->get();

            $usedLookup = $usedData->groupBy('subdistrict_id')
                ->map(function ($detailsGroup) {
                    return $detailsGroup->map(function ($detail) {
                        return ($detail->region && $detail->region->reg_name) ? $detail->region->reg_name : null;
                    })
                        ->filter()
                        ->unique()
                        ->implode(', ');
        ...
        $subDistricts->each(function ($sub) use ($usedLookup) {

            if (array_key_exists($sub->id, $usedLookup)) {
                $sub->is_used = true;
                $sub->used_by_region_name = $usedLookup[$sub->id];
            } else {
                $sub->is_used = false;
                $sub->used_by_region_name = null;
            }
        });
    ...
    public function removeSubdistrictUsage(Request $request)
    {
        $request->validate([
            'subdistrict_id' => 'required|integer|exists:master_sub_districts,id',
        ]);
        ...
        try {
            ...
            $updatedCount = DeliveryRegionDetail::where('subdistrict_id', $subdistrictId)
                ->where('status', 1)
                ->update([
                    'status' => 0,
                    'updated_at' => now(),
            ...
            if ($updatedCount > 0) {
                return response()->json([
                    'success' => true,
                    'message' => "นำตำบลออกจากเขตการขายเดิม ($updatedCount รายการ) เรียบร้อยแล้ว"
                ]);
            } else {
                return response()->json([
                    'success' => true,
                    'message' => "ไม่พบการใช้งานตำบลนี้ในเขตที่ Active อยู่ (อาจถูกลบไปแล้ว)"
                ]);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error("Error removing subdistrict usage: " . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'เกิดข้อผิดพลาดทางเทคนิค ไม่สามารถลบข้อมูลได้'
            ], 500);
        }
    }
    ...
    public function saveShippingRateProfile(Request $request)
    {
        ...
        // 1. ตรวจสอบว่าหา ID เจอหรือไม่ ป้องกัน Error หาก ID เป็น Null
        $shippingProfileData = ShippingProfile::find($request->shipping_profile_id);
        if (!$shippingProfileData) {
            return redirect()->back()->with('error', 'Shipping Profile not found.');
        }
        ...
        // --- จัดการ CSV ---
        $is_csv_imported = array(); // ประกาศตัวแปรไว้ก่อนเสมอ
        ...
        try {
            if ($shippingProfileData->save()) {
                ...
                // ตรวจสอบว่ามีการส่งข้อมูล region_name หรือ cutoff_time มาไหม
                if ($request->filled('region_name') && $request->filled('cutoff_time')) {
                    // เรียกใช้ Model DeliveryTime ให้ทำงาน
                    \App\DeliveryTime::updateDeliveryTime($request);
                }
                // ============================================================
                ...
                /* update activity log */
                $logdata = array('action_type' => "Edit", 'module_name' => "Shipping Table Rates", 'logdetails' => "Admin has updated shipping rate profile", 'old_data' => "", 'new_data' => "");
                $this->updateLogActivity($logdata);

                // --- [แก้ไขจุดสำคัญ 2] กำหนด return_url กรณีสำเร็จ ---
                ...
                // กรณี Save ไม่สำเร็จ (แต่ไม่มี Exception)
                $return_url = redirect()->back()->with('error', 'No changes made or save failed.');
            }
        } catch (\Exception $e) {
            // กรณีเกิด Error จริงๆ เช่น SQL Error
            return redirect()->back()->with('error', 'Error Saving: ' . $e->getMessage());
        }

        return $return_url;
    }

    // public function checkDuplicateZone(Request $request) {
    //     // ---------------------------------------------------------
    //     // 1. รับค่า Input และแปลงเป็น Array
    //     // ---------------------------------------------------------
    //     $reqProvinces = $request->input('provinces');
    //     $reqDistricts = $request->input('districts');
    //     $reqSubDistricts = $request->input('selected_subdistricts');
    //     $regType = $request->input('deliveryType'); // ประเภทเขตการขาย (3=จัดส่งถึงบ้าน, 2=รับที่จุดรับสินค้า)

    //     if ($regType == 'address') {
    //         $reg_type = 3;
    //     } elseif ($regType == 'pickup') {
    //         $reg_type = 2;
    //     }

    //     $provinceIds = !empty($reqProvinces) ? (is_array($reqProvinces) ? $reqProvinces : explode(',', $reqProvinces)) : [];
    //     $districtIds = !empty($reqDistricts) ? (is_array($reqDistricts) ? $reqDistricts : explode(',', $reqDistricts)) : [];
    //     $subDistrictIds = !empty($reqSubDistricts) ? (is_array($reqSubDistricts) ? $reqSubDistricts : explode(',', $reqSubDistricts)) : [];

    //     // 2.1 กรณีเลือกจังหวัด (ต้องหา District ก่อน แล้วค่อยหา SubDistrict)
    //     if (!empty($provinceIds)) {
    //         // ขั้นที่ 1: หา District ID ทั้งหมดในจังหวัดพวกนี้
    //         $distIdsInProvinces = MasterDistrict::whereIn('province_id', $provinceIds)
    //                                 ->pluck('id')
    //                                 ->toArray();

    //         // ขั้นที่ 2: หา SubDistrict ID จาก District ที่ได้มา
    //         if (!empty($distIdsInProvinces)) {
    //             $subsFromProvinces = MasterSubDistrict::whereIn('district_id', $distIdsInProvinces)
    //                                             ->pluck('id')
    //                                             ->toArray();

    //             // เอามารวมกับ array หลัก
    //             $subDistrictIds = array_merge($subDistrictIds, $subsFromProvinces);
    //         }
    //     }

    //     // 2.2 กรณีเลือกอำเภอ (หา SubDistrict จาก district_id ได้เลย)
    //     if (!empty($districtIds)) {
    //         $subsFromDistricts = MasterSubDistrict::whereIn('district_id', $districtIds)
    //                                 ->pluck('id')
    //                                 ->toArray();

    //         $subDistrictIds = array_merge($subDistrictIds, $subsFromDistricts);
    //     }

    //     // 2.3 ตัดตัวซ้ำออก
    //     $finalSubDistrictIds = array_values(array_unique($subDistrictIds));

    //     // ถ้าไม่มี ID ตำบลเลย ให้จบการทำงาน
    //     if (empty($finalSubDistrictIds)) {
    //         return response()->json(['is_duplicate' => false]);
    //     }

    //     // ---------------------------------------------------------
    //     // 3. Query Database เพื่อเช็คซ้ำ
    //     // ---------------------------------------------------------
    //     $currentRegionId = $request->input('reg_id') ?? $request->input('shipping_profile_id');

    //     $duplicates = DB::table('delivery_region_detail as detail')
    //         ->join('delivery_region as main', 'detail.region_id', '=', 'main.reg_id')

    //         // Join 1: ตำบล -> อำเภอ
    //         ->leftJoin('master_sub_districts as sub', 'detail.subdistrict_id', '=', 'sub.id')
    //         // Join 2: อำเภอ -> จังหวัด (เพื่อเอา ID ไปสั่ง Uncheck ตัวแม่)
    //         ->leftJoin('master_districts as dist', 'sub.district_id', '=', 'dist.id')

    //         ->whereIn('detail.subdistrict_id', $finalSubDistrictIds)

    //         // --- เช็คเฉพาะประเภทเดียวกัน (reg_type) ---
    //         ->when($reg_type, function($q) use ($reg_type) {
    //             return $q->where('main.reg_type', $reg_type);
    //         })

    //         ->when($currentRegionId, function($q) use ($currentRegionId) {
    //             return $q->where('detail.region_id', '!=', $currentRegionId);
    //         })
    //         ->where('detail.status', 1) 
    //         ->where('main.status', 1)
    //         ->select(
    //             'detail.subdistrict_id',
    //             'main.reg_name as profile_name',
    //             'sub.name_th as sub_name_th',
    //             'sub.zip_code',
    //             'sub.district_id',
    //             'dist.province_id'
    //         )
    ...
    //         // สร้าง Array เก็บ ID ของตัวพ่อและตัวปู่ที่จะต้องเอาติ๊กออก
    //         $conflictDistrictIds = [];
    //         $conflictProvinceIds = [];

    //         foreach ($duplicates as $dup) {
    //             if (!in_array($dup->subdistrict_id, $duplicateIds)) {
    //                 $duplicateIds[] = $dup->subdistrict_id;
    //             }

    //             // เก็บ ID อำเภอ และ จังหวัด ที่เกี่ยวข้อง
    //             if ($dup->district_id && !in_array($dup->district_id, $conflictDistrictIds)) {
    //                 $conflictDistrictIds[] = $dup->district_id;
    //             }
    //             if ($dup->province_id && !in_array($dup->province_id, $conflictProvinceIds)) {
    //                 $conflictProvinceIds[] = $dup->province_id;
    //             }

    //             $locationName = $dup->sub_name_th 
    //                             ? "{$dup->sub_name_th} ({$dup->zip_code})" 
    //                             : "ตำบล ID: {$dup->subdistrict_id}";

    //             $details[] = [
    //                 'location_name' => $locationName,
    //                 'conflict_profile_name' => $dup->profile_name
    //             ];
    //         }
    ...
    //             'conflict_district_ids' => $conflictDistrictIds,
    //             'conflict_province_ids' => $conflictProvinceIds
    //         ]);
    //     }
    ...
    public function checkDuplicateZone(Request $request)
    {
        $reqGeographies = $request->input('selected_geographies');
        $reqProvinces = $request->input('selected_provinces');
        $reqDistricts = $request->input('selected_districts');
        $reqSubDistricts = $request->input('selected_subdistricts');
        $regType = $request->input('deliveryType');

        $reg_type = ($regType == 'address') ? 3 : (($regType == 'pickup') ? 2 : null);

        $geoIds = !empty($reqGeographies) ? (is_array($reqGeographies) ? $reqGeographies : explode(',', $reqGeographies)) : [];
        $provinceIds = !empty($reqProvinces) ? (is_array($reqProvinces) ? $reqProvinces : explode(',', $reqProvinces)) : [];
        $districtIds = !empty($reqDistricts) ? (is_array($reqDistricts) ? $reqDistricts : explode(',', $reqDistricts)) : [];
        $subDistrictIds = !empty($reqSubDistricts) ? (is_array($reqSubDistricts) ? $reqSubDistricts : explode(',', $reqSubDistricts)) : [];

        $allTargetSubDistrictIds = $subDistrictIds;


        if (!empty($geoIds)) {
            $subsFromGeos = DB::table('master_sub_districts as sub')
                ->join('master_districts as dist', 'sub.district_id', '=', 'dist.id')
                ->join('master_provinces as prov', 'dist.province_id', '=', 'prov.id')
                ->whereIn('prov.geography_id', $geoIds)
                ->pluck('sub.id')
                ->toArray();
            $allTargetSubDistrictIds = array_merge($allTargetSubDistrictIds, $subsFromGeos);
        }

        if (!empty($provinceIds)) {
            $subsFromProvinces = DB::table('master_sub_districts as sub')
                ->join('master_districts as dist', 'sub.district_id', '=', 'dist.id')
                ->whereIn('dist.province_id', $provinceIds)
                ->pluck('sub.id')
                ->toArray();
            $allTargetSubDistrictIds = array_merge($allTargetSubDistrictIds, $subsFromProvinces);
        }

        if (!empty($districtIds)) {
            $subsFromDistricts = DB::table('master_sub_districts')
                ->whereIn('district_id', $districtIds)
                ->pluck('id')
                ->toArray();
            $allTargetSubDistrictIds = array_merge($allTargetSubDistrictIds, $subsFromDistricts);
        }

        $finalSubDistrictIds = array_values(array_unique($allTargetSubDistrictIds));
        ...
        if ($duplicates->count() > 0) {
            ...
            foreach ($duplicates as $dup) {
                ...
                $details[] = [
                    'location_name' => $dup->sub_name_th ? "{$dup->sub_name_th} ({$dup->zip_code})" : "ตำบล ID: {$dup->subdistrict_id}",
                    'conflict_profile_name' => $dup->profile_name
                ];
            }
    ...
    {
        $region = \App\DeliveryRegion::find($id);
        if (!$region) {
            return response()->json(['status' => 'error', 'message' => 'Not found']);
        }

        /** ===============================
         * 1. Selected Subdistrict IDs
         * =============================== */
        $selectedSubIds = \App\DeliveryRegionDetail::where('region_id', $id)
            ->pluck('subdistrict_id')
            ->toArray();

        /** ===============================
         * 2. หา hierarchy จาก subdistrict ที่เลือก
         * =============================== */
        $selectedSubs = \App\MasterSubDistrict::with('district.province.geography')
        ...
        foreach ($selectedSubs as $sub) {
            ...
            $selectedIds['districts'][$dist->id]   = $dist->id;
            $selectedIds['provinces'][$prov->id]   = $prov->id;
            $selectedIds['geographies'][$geo->id]  = $geo->id;
        }

        // แปลงเป็น array ปกติ
        $selectedIds = [
            'geographies'  => array_values($selectedIds['geographies']),
            'provinces'    => array_values($selectedIds['provinces']),
            'districts'    => array_values($selectedIds['districts']),
            'subdistricts' => $selectedIds['subdistricts'],
        ];

        /** ===============================
         * 3. FULL STRUCTURE (ทั้งหมด)
         * =============================== */
        $fullStructure = [
            'geographies' => \App\MasterGeography::select('id', 'name')->get(),
            'provinces'   => \App\MasterProvince::select('id', 'name_th', 'geography_id')->get(),
            'districts'   => \App\MasterDistrict::select('id', 'name_th', 'province_id')->get(),
            'subdistricts' => \App\MasterSubDistrict::select('id', 'name_th', 'district_id', 'zip_code')->get(),
        ];

        /** ===============================
         * 4. Time Slots
         * =============================== */
        $timeSlots = \App\DeliveryTimeSlot::where('reg_id', $id)->get();
        $formattedSlots = [];

        $calcTime = function ($baseTime, $diffMinutes) {
            if (is_null($diffMinutes)) return '';
            return date('H:i', strtotime($baseTime) + ($diffMinutes * 60));
        };
        ...
        foreach ($timeSlots as $slot) {
            ...
            $temp['seller_start_time_str'] = $calcTime($cutoff, $slot->seller_start_deli_time);
            $temp['seller_end_time_str']   = $calcTime($cutoff, $slot->seller_end_deli_time);
            $temp['start_deli_time_str']   = $calcTime($cutoff, $slot->start_deli_time);
            $temp['end_deli_time_str']     = $calcTime($cutoff, $slot->end_deli_time);
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'dc_address'   => $region->dc_address,
                'reg_type'     => $region->reg_type,
                'status'       => $region->status,
                'dc_tel'       => $region->dc_tel,
    ...
    public function deleteRegion($id)
    {
        ...
        try {
            ...
            if (!$region) {
                return response()->json(['status' => 'error', 'message' => 'ไม่พบข้อมูลที่ต้องการลบ']);
            }

            $affectedProvinceIds = DB::table('delivery_region_detail')
                ...
                ->where('delivery_region_detail.region_id', $id)
                ->distinct()
                ->pluck('master_districts.province_id');

            \App\DeliveryRegionDetail::where('region_id', $id)->update(['status' => 0]);
            \App\DeliveryTimeSlot::where('reg_id', $id)->update(['status' => 0]);

            $region->update([
                'st_del' => 1,
                'del_by' => auth()->user()->id ?? 0,
                'del_at' => now(),
                'status' => 0
            ]);

            if ($affectedProvinceIds->isNotEmpty()) {
                foreach ($affectedProvinceIds as $pId) {
                    $isStillUsed = DB::table('delivery_region_detail')
                        ...
                        ->where('delivery_region.reg_id', '!=', $id)
            ...
            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'ลบข้อมูลโซนการจัดส่งเรียบร้อยแล้ว']);
    ...
    public function fielddata($datarray)
    {
        ...
        $marks = array(
            'fieldSets' => array(
                ...
                array(
                    "fieldName" => "status",
                    "sortable" => $replace_arr[$status->sort],
                    "filterable" => $replace_arr[$status->filter],
                    'width' => $status->width,
                    'align' => $status->align,
                    "fieldType" => "selectbox",
