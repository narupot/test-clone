...
{   
    ...
    public function show(Request $request) {
        ...
        $google_maps_api_key = env('GOOGLE_MAPS_API_KEY', 'AIzaSyCBYiKxZ3HYjLO7vZVa2n0f3_QuZj2lC9g');
        $def_country_dtl = GeneralFunctions::getDefaultCountryDetail();
        $use_smm_address = (!empty($def_country_dtl) && $def_country_dtl->short_code == 'TH');

        return view('shipBillAddress.addressList', [
            'billing_add' => $billing_address,
            'shipping_add' => $shipping_address,
            'all_address' => $all_address,
            'google_api_key' => $google_maps_api_key,
            'map_default_lat' => 13.9562464,
            'map_default_long' => 100.6154008,
            'use_smm_address' => $use_smm_address
        ]);
    }
    ...
    function edit(Request $request, $address_id) {
        ...
        if($request->call_type == 'ajax_data') {
            ...
            // อัปเดตพิกัดจากแผนที่เมื่อส่ง lat/long มา
            ...
            return view('shipBillAddress.addressEdit', [
                'address' => $address,
                'def_country_dtl' => $def_country_dtl,
                'use_smm_address' => $use_smm_address
            ]);
    ...
    public function showMapPicker(Request $request) {
        // รองรับทั้ง call_type=ajax_data และการเรียกโดยตรง
        $lat = $request->lat ?? 13.9562464; // พิกัดเริ่มต้นแผนที่
        $long = $request->long ?? 100.6154008;
        $addressId = $request->address_id ?? null;
        $google_maps_api_key = env('GOOGLE_MAPS_API_KEY', 'AIzaSyCBYiKxZ3HYjLO7vZVa2n0f3_QuZj2lC9g');
        
        $editAddressUrl = $addressId ? action('User\UserController@edit', $addressId) : null;
        
        return view('shipBillAddress.mapPicker', [
            'default_lat' => $lat,
            'default_long' => $long,
            'google_api_key' => $google_maps_api_key,
