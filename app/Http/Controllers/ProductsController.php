...
class ProductsController extends MarketPlace
{
    ...
    /**
     * Log empty search queries into `smm_search_log`.
     */
    private function logSearchTerm($text, Request $request)
    {
        if (empty($text)) {
            return;
        }
        try {
            ...
            \Log::error('Failed to insert search log: ' . $e->getMessage());
        }
    }
    ...
    public function category(Request $request, $url)
    {
        ...
        if ($product_list->total() == 0) {
            $this->logSearchTerm($search, $request);
        }
    ...
    public function searchProductsByTag(Request $request)
    {
        ...
        $page_item  = $request->input('itemsPerPage', 20);
        ...
        $tagMatch = \App\MongoProductTypeTag::where('tag_status', 1)
            ->where('tag', 'like', "%{$keyword}%")
            ->pluck('product_type_id')
            ->toArray();

        if (empty($tagMatch)) {
            $this->logSearchTerm($keyword, $request);
            return response()->json([
                'status' => 'fail',
        ...
        $products = \App\MongoProduct::whereIn('cat_id', $tagMatch) 
            ->where('status', 1)
            ->orderBy($order_by, $order)
            ->paginate($page_item);
    ...
    public function getProductsbycategory(Request $request)
    {
        ...
        if (!is_null($filter_attributes)) {
            ...
            $query->with('shop')->with('badge')->where('cat_id', $cat_id)->whereNotIn('shop_id', $shop_closed_id)->where('status', '1');
    ...
    //         // if(!$p_cate){ return redirect()->action('ProductsController@search', ['search' => $search]); }
    //     }

    //     $p_type = null;
    //     if($req_product_type){
    //         $p_type = \App\MongoCategory::where('url',$req_product_type)->first();
    //         // if(!$p_type){ return redirect()->action('ProductsController@search', ['search' => $search]); }
    //     }
    ...
    //     $categoryUrl = action('ProductsController@search');
    //     $product_type->each(function ($category) use ($categoryUrl) {
    //         $category->category_image = $category->img?getCategoryImageUrl($category->img):null;
    //         $catslug = $category->url;
    //         if($category->parent_id == 0){
    //             $category->url = $categoryUrl."?productCate=".$catslug;
    //         }else{
    //             $category->url = $categoryUrl."?productType=".$catslug;
    //         }
    //     });
    ...
    //     $product_search_1_sort = (clone $products_base)->when($search, function ($query, $search) {
    //         return $query->whereHas('category', function ($q) use ($search) {
    //             $q->where('status','1')->where('category_name', 'like', $search . '%');
    //         });
    //     })->get();

    //     $product_search_2_sort = (clone $products_base)->when($search, function ($query, $search) {
    //         return $query->whereHas('category', function ($q) use ($search) {
    //             $q->where('status','1')->where('category_name', 'like', '%' . $search .'%');
    //         });
    //     })->get();
    ...
    //     if($sortBy==1){
    //         $product_search_1 = $product_search_1_sort->sortBy("unit_price");
    //         $product_search_2 = $product_search_2_sort->sortBy("unit_price");
    //     }elseif($sortBy==2){
    //         $product_search_1 = $product_search_1_sort->sortByDesc("unit_price");
    //         $product_search_2 = $product_search_2_sort->sortByDesc("unit_price");
    //     }elseif($sortBy==3){
    //         $product_search_1 = $product_search_1_sort->sortBy("category_name");
    //         $product_search_2 = $product_search_2_sort->sortBy("category_name");
    //     }elseif($sortBy==4){
    //         $product_search_1 = $product_search_1_sort->sortByDesc("category_name");
    //         $product_search_2 = $product_search_2_sort->sortByDesc("category_name");
    //     }else{
    //         $product_search_1 = $product_search_1_sort->sortBy("unit_price");
    //         $product_search_2 = $product_search_2_sort->sortBy("unit_price");
    //     }
    ...
    //     return view('searchProductShopList', 
    //         [
    //             // 'badges' => $badges, 
    //             'status' => 'success',
    //             'product_list' => $product_list,
    ...
    public function search(Request $request)
    {
        ...
        $p_cate = null;
        if ($req_product_cate) {
            $p_cate = \App\ParentCategory::where('url', $req_product_cate)->first();
        }
   
        $p_type = null;
        if ($req_product_type) {
            $p_type = \App\Category::where('url', $req_product_type)->first();
        }
        ...
        $req_badges = \App\MongoBadge::query()
            ...
            if (!empty($search)) {
                try {
                    $isEnglish = preg_match('/^[a-zA-Z0-9\s]+$/u', $search);
 
                    if ($isEnglish) {
                        $tagProductTypeIds = \App\MongoProductTypeTag::raw(function($collection) use ($search) {
                            return $collection->find(
                                ...
                                [
                                    'limit' => 200,
                                    'projection' => ['tag' => 1, 'product_type_id' => 1, '_id' => 0]
                                ]
                            );
                        })
                        ->map(function ($item) {
                        ...
                        $isThai = preg_match('/[ก-๙]/u', $search);
 
                        if ($isThai) {
                            if (mb_strlen($search, 'UTF-8') <= 2) {
                                // คำสั้น: match เฉพาะขอบคำหรือขึ้นต้น/ลงท้าย
                                $regex = "(^|\\s)" . $escaped . "(\\s|$)";
                            } else {
                                // คำยาว: match ได้ทุกตำแหน่งในคำ
                                $regex = $escaped;
                            }
                        } else {
                            // ภาษาอังกฤษ: ใช้ word boundary
                            $regex = "(^|\\s|\\b)" . $escaped . "(\\b|\\s|$)";
                        }
 
                        $thaiCollation = ['locale' => 'th', 'strength' => 1];
 
                        $tagProductTypeIds = \App\MongoProductTypeTag::raw(function($collection) use ($regex, $thaiCollation) {
                            return $collection->find(
                                [
                                    'tag_status' => 1,
                                    'tag' => ['$regex' => $regex, '$options' => 'im']
                                ],
                                [
                                    'limit' => 300,
                                    'projection' => ['tag' => 1, 'product_type_id' => 1, '_id' => 0],
                                    'collation' => $thaiCollation
                                ]
                            );
                        })
                        ->map(function ($item) {
                    ...
                    $tagProductTypeIds = $tagProductTypeIds
                        ->filter(fn($item) => !empty($item['product_type_id']))
                        ->unique('product_type_id')
                        ->sort(function ($a, $b) use ($search) {
                            $aExact = mb_strtolower($a['tag']) === mb_strtolower($search) ? 0 : 1;
                            $bExact = mb_strtolower($b['tag']) === mb_strtolower($search) ? 0 : 1;
                            if ($aExact !== $bExact) return $aExact - $bExact;
 
                            $aStarts = stripos($a['tag'], $search) === 0 ? 0 : 1;
                            $bStarts = stripos($b['tag'], $search) === 0 ? 0 : 1;
                            if ($aStarts !== $bStarts) return $aStarts - $bStarts;
 
                            return stripos($a['tag'], $search) - stripos($b['tag'], $search);
                        })
                        ->pluck('product_type_id')
                        ->take(500)
                        ->values()
                    ...
                    \Log::error("Error in tagProductTypeIds search: " . $e->getMessage());
                    $tagProductTypeIds = [];
                }
            }
        $page = (int) ($request->get('page', 1));
        $per_page = (int) ($request->get('perPage', 48));
       
        if (empty($tagProductTypeIds) && empty($p_cate) && empty($p_type)) {
            $emptyPaginator = new LengthAwarePaginator([], 0, $per_page, $page, [
                'path' => request()->url(),
                'query' => request()->query(),
            ]);
        ...
        $filterProductTypeIds = !empty($tagProductTypeIds) ? $tagProductTypeIds : null;
       
        $product_type_limit = 200;
 
        $product_type_base = \App\MongoCategory::query()
            ->when($filterProductTypeIds, function ($query) use ($filterProductTypeIds) {
                return $query->whereIn('_id', $filterProductTypeIds);
            })
            ->when($p_cate, function ($query) use ($p_cate) {
                return $query->where('parent_id', $p_cate->id ?? null);
            })
            ->when($p_type, function ($query) use ($p_type) {
                return $query->where('id', $p_type->id ?? null);
            })
            ->select('category_name', 'img', 'url', 'parent_id')
            ->whereHas('product', function ($q) {
                // $q->where('quantity', '>', 0)
        ...
        $parentIds = $product_type_base->pluck('parent_id')->filter()->toArray();
 
        $parents = \App\ParentCategory::with(['group', 'subgroup'])
            ->whereIn('id', $parentIds)
            ->get()
            ->keyBy('id');
 
        // \Log::debug('==== ก่อนเรียง product_type ====');
        // foreach ($product_type_base as $cat) {
        //     $parent = $parents[$cat->parent_id] ?? null;
        //     $group_sort = $parent->group->sorting_no ?? 99999;
        //     $subgroup_sort = $parent->subgroup->sorting_no ?? 99999;
        //     $parent_sort = $parent->sorting_no ?? 99999;
        //     $self_sort = $cat->sorting_no ?? 99999;
 
        //     $sorting_key = sprintf("%05d-%05d-%05d-%05d", $group_sort, $subgroup_sort, $parent_sort, $self_sort);
        //     \Log::debug("BeforeSort - Category: {$cat->category_name}, Key: {$sorting_key}");
        // }
 
        $product_type = $product_type_base->map(function ($cat) use ($parents) {
            $parent = $parents[$cat->parent_id] ?? null;
            $group_sort    = $parent->group->sorting_no ?? 99999;
            $subgroup_sort = $parent->subgroup->sorting_no ?? 99999;
            $parent_sort   = $parent->sorting_no ?? 99999;
            ...
            $cat->sorting_key = sprintf("%05d-%05d-%05d-%05d", $group_sort, $subgroup_sort, $parent_sort, $self_sort);
 
            $categoryUrl = action('ProductsController@search');
            $catslug = $cat->url;
            $cat->url = $cat->parent_id == 0
                ? "{$categoryUrl}?productCate={$catslug}"
                : "{$categoryUrl}?productType={$catslug}";
            $cat->category_image = $cat->img ? getCategoryImageUrl($cat->img) : null;
        ...
        $product_type = $product_type
        ->sort(function ($a, $b) {
            $result = strnatcmp($a['sorting_key'], $b['sorting_key']);
            if ($result === 0) {
                return strcasecmp($a['category_name'] ?? '', $b['category_name'] ?? '');
            }
            return $result;
        })
        ->take($product_type_limit)
        ->values();
 
        // \Log::debug('==== หลังเรียง product_type ====');
        // foreach ($product_type as $cat) {
        //     \Log::debug("AfterSort - Category: {$cat->category_name}, Key: {$cat->sorting_key}");
        // }
        ...
        $shops_base = \App\MongoShop::query()
            ->when($p_cate, function ($query) use ($p_cate) {
                return $query->whereHas('product', function ($q) use ($p_cate) {
                    ...
                    // ->where('quantity', '>', 0)
                    ->whereHas('category', function ($qq) use ($p_cate) {
                        $qq->where('parent_id', $p_cate->id ?? null);
                    });
                });
            })
            ->when($p_type, function ($query) use ($p_type) {
                return $query->whereHas('product', function ($q) use ($p_type) {
                    $q->where('status', '1')
                    // ->where('quantity', '>', 0)
                    ->where('cat_id', $p_type->id ?? null);
                });
            })
            ->when($filterProductTypeIds, function ($query) use ($filterProductTypeIds) {
                return $query->whereHas('product', function ($q) use ($filterProductTypeIds) {
                    $q->whereIn('cat_id', $filterProductTypeIds)
                    // ->where('quantity', '>', 0)
                    ->where('status', '1');
                });
            })
        ...
        $shop_list = $shops_base->unique('_id')->values();
        ...
        $product_size = \App\MongoSizeGrade::where('type', 'size')->where('status', '1')->get();
        $product_grade = \App\MongoSizeGrade::where('type', 'grade')->where('status', '1')->get();
        ...
        $products_base = \App\MongoProduct::query()
            ...
            ->when($p_cate, function ($query) use ($p_cate) {
                return $query->whereHas('category', function ($q) use ($p_cate) {
                    $q->where('parent_id', $p_cate->id ?? null);
                });
            })
            ->when($p_type, function ($query) use ($p_type) {
                return $query->whereHas('category', function ($q) use ($p_type) {
                    $q->where('_id', $p_type->id ?? null);
                });
            })
            ->when($filterProductTypeIds, function ($query) use ($filterProductTypeIds) {
                return $query->whereIn('cat_id', $filterProductTypeIds);
            })
            ->when(Auth::check(), function ($query) {
                $query->with('wishlist');
            })
        ...
        $all_products = $products_base->get();
       
        if ($sortBy == 1) {
            $sorted_products = $all_products->sortBy("unit_price");
        } elseif ($sortBy == 2) {
            $sorted_products = $all_products->sortByDesc("unit_price");
        } elseif ($sortBy == 3) {
            $sorted_products = $all_products->sortBy(function ($product) {
        ...
        $merged = $sorted_products->unique('_id')->values();
        ...
        $product_list->each(function ($product) {
            ...
            $product->badge_img = $product->badge_id ? getBadgeImage($product->badge_id) : null;
            $product->cate_name = $product->category->category_name ?? null;
            $product->cate_url  = $product->category->url ?? null;
            $product->package_name = $product->package_id ? getPackageName($product->package_id) : null;
        });
    ...
    public function getProductsBysearch(Request $request)
    {
        ...
        if (empty($product_data['data'] ?? [])) {
            $this->logSearchTerm($name, $request);
        }
    ...
    public function getProductsShopBysearch(Request $request)
    {
        ...
        $data_prd = \App\Product::select(DB::raw('min(unit_price) as minunit'), 'cat_id')->where('status', '1')->whereIn('cat_id', $res_cat_id)->whereNotIn('shop_id', $shop_closed_id)->groupby('cat_id')->orderBy('minunit', 'asc')->get()->toArray();
        $main_res_data = [];
        $i = 0;
    ...
    public function autosearch(Request $request)
    {
        ...
        if ($request->searchtype == 'all') {
            ...
            foreach ($product_data as $key => $result) {
                ...
                $data[$i]['url'] = action('ProductsController@search', ['search' => $result['category_name']]);
    ...
    public function viewAllProductsBySlider(Request $request)
    {
        
        $slider_id = $request->slider;
        $perPage = $request->perPage ?? 30;
        // บังคับเรียงตามราคาถูกไปแพง
        $order_by = 'unit_price';
        $order = 'asc';
        
        // Get slider details
        $slider = \App\CmsSlider::find($slider_id);
        if (!$slider) {
            abort(404);
        }

        // Get products based on slider name
        if ($slider->name === 'product-for-you') {
            $products = \App\CmsSlider::productForYou($slider);
        } else if ($slider->name === 'flash-sales') {
            $products = \App\CmsSlider::flashSales($slider);
        } else if ($slider->name === 'ส-นค-าแนะนำ' || $slider->name === 'fresh-fruits') {
            $products = \App\CmsSlider::recommendedProducts($slider);
        } else {
            abort(404);
        }

        $formatted_products = collect($products)->map(function($product) {
            // Parse URL to get category URL and SKU
            $url_parts = parse_url($product['cat_url']);
            $path_parts = explode('/', trim($url_parts['path'], '/'));
            $sku = end($path_parts);
            $cat_url = prev($path_parts);

            // หาชื่อร้านค้า ถ้ามีข้อมูล shop_name ใน $product ให้ใช้เลย ถ้าไม่มีก็ใส่เป็นค่าว่าง
            $shop_name = isset($product['shop_name']) ? $product['shop_name'] : '';

            return (object)[
                'id' => $product['_id'],
                'name' => $product['cat_name'],
                'cate_name' => $product['cat_name'], // สำหรับ component product-card 
                'product_image' => $product['cat_img'],
                'shop_name' => $shop_name,
                'unit_name' => $product['base_unit'],
                'badge_img' => $product['badge_img'],
        ...
        // เรียงสินค้าตามราคาถูกไปแพง
        $sorted_products = $formatted_products->sortBy('unit_price')->values()->all();

        // Create paginator manually
        $page = $request->get('page', 1);
        $offset = ($page - 1) * $perPage;
        $items = array_slice($sorted_products, $offset, $perPage);
        
        $product_list = new \Illuminate\Pagination\LengthAwarePaginator(
            $items,
            count($sorted_products),
            $perPage,
            $page,
            ['path' => $request->url(), 'query' => $request->query()]
        );

        if($slider->name === 'ส-นค-าแนะนำ' || $slider->name === 'fresh-fruits'){
             return view('products.slider-cat', [
            ...
            return view('products.slider-products', [
                'products' => $product_list,
                'slider' => $slider,
                'show_per_page' => json_encode(getShowRangePerPage()),
                'order_by_item' => json_encode(getSortingItems()),
    ...
    //         $tags = \App\MongoProductTypeTag::where('tag', 'regexp', "/$keyword/i")
    //                 ->where('tag_status', 1)
    //                 ->limit(10)
    //                 ->pluck('tag')
    //                 ->toArray(); 
    ...
    //     } catch (\Exception $e) {
    //         \Log::error("Error in searchTags: " . $e->getMessage());
    //         return response()->json([]);
    //     }
    // }
    ...
    public function searchTags(Request $request)
    {
        ...
        try {
            $isEnglish = preg_match('/^[a-zA-Z0-9\s]+$/u', $keyword);

            if ($isEnglish) {
                $tags = \App\MongoProductTypeTag::raw(function($collection) use ($keyword) {
                    return $collection->find(
                        [
                            '$text' => [
                                '$search' => $keyword
                ...
                $regexExact  = "(?:^|\s)$escaped(?:\s|$)";
                $regexPrefix = "^$escaped";

                $tags = \App\MongoProductTypeTag::raw(function($collection) use ($regexExact, $regexPrefix) {
                    $thaiCollation = ['locale' => 'th', 'strength' => 1];
                    ...
                    return $collection->find(
                        [
                            'tag_status' => 1,
                            '$or' => [
                                ['tag' => ['$regex' => $regexExact, '$options' => 'im']],
                                ['tag' => ['$regex' => $regexPrefix, '$options' => 'im']],
                            ]
                        ],
                        [
                            'limit' => 50,
            ...
            // \Log::info("searchTags - ก่อนเรียง", ['keyword' => $keyword, 'tags_before' => $tags]);

            $tags = array_values(array_unique($tags));

            usort($tags, function ($a, $b) use ($keyword) {
                $aLower = mb_strtolower($a);
                $bLower = mb_strtolower($b);
                $keywordLower = mb_strtolower($keyword);

                $aExact = $aLower === $keywordLower ? 0 : 1;
                $bExact = $bLower === $keywordLower ? 0 : 1;
                if ($aExact !== $bExact) return $aExact - $bExact;

                $aStarts = stripos($a, $keyword) === 0 ? 0 : 1;
                $bStarts = stripos($b, $keyword) === 0 ? 0 : 1;
                if ($aStarts !== $bStarts) return $aStarts - $bStarts;

                return stripos($a, $keyword) - stripos($b, $keyword);
            });

            // \Log::info("searchTags - หลังเรียง", ['keyword' => $keyword, 'tags_after' => $tags]);
