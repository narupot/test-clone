...
class OrderController extends MarketPlace {
    ...
    public function orderHistoryData(Request $request){
        ...
        $order_data= DB::table(with(new OrderShop)->getTable().' as os')
                ...
                ->select('os.end_shopping_date', 'os.order_id','os.shop_formatted_id','os.user_name','os.shipping_method','osd.status')
                ...
                // P แก้ ให้ cancelled ออก start
                    ...
                    $query->whereIn('os.order_status',[2,3]);
                    $query->orWhere('o.pickup_time','<',$check_date);
                })
                ->where('os.order_status','!=',4); // Exclude cancelled orders
        ...
        if(!empty($order_list)){
            ...
            foreach ($order_list as $ord_val){
                ...
                $nestedData['shop_formatted_id'] = '<a href="'.action('Seller\OrderController@details',$ord_val->shop_formatted_id).'" class="link-skyblue">'.mb_substr($ord_val->order_id, -4).'</a>';
    ...
    public function deliveryListData(Request $request){
        ...
        ***/
        ...
        $order_data = DB::table(with(new \App\OrderShop)->getTable().' as os')
                ...
                ->orderBy('os.id', 'desc')
                ->select('os.user_name','os.order_id','os.shop_formatted_id','os.order_status','os.shipping_method','os.end_shopping_date','os.total_final_price', 'u.image','osd.status','ord.pickup_time');
        ...
        if($request->section=='ready'){
            $order_data->where(function($query) use($check_date,$prefix){
                ...
                **/
                $query->whereIn('os.order_status',[2])->where('os.payment_status',1)->where('os.seller_status','sent');
        ...
        $pickup = \App\DeliveryTime::getDeliveryTime();
        $pickup_time =0;
        if($pickup){
        ...
        if(!empty($order_list)){
            foreach ($order_list as $ord_val){
                ...
                $ord_val->pickup_time=$ord_val->pickup_time ? date('Y-m-d H:i',strtotime($ord_val->pickup_time)-$pickup_time*60*60) : '';
    ...
    public function orderOutstandingBalanceData(Request $request){
        $column_arr = ['id','order_id','shop_formatted_id','user_name','total_final_price'];
        $draw        = $request->draw;
        $start       = $request->start; //Start is the offset
        $length      = $request->length; //How many records to show
        $column      = $request->order[0]['column']; //Column to orderBy
