...
class OrderController extends MarketPlace {
    ...
    public static function saveFinalOrder($orderId){
        ...
        $dc_delivery_starttime = null;
        $dc_delivery_endtime   = null;

        if (!empty($orderInfo->del_t_s_id)) {
            $timeSlot = \App\DeliveryTimeSlot::where('del_t_s_id', $orderInfo->del_t_s_id)
                        ->where('status', 1)
                        ->first();

            if ($timeSlot) {
                $now = Carbon::now('Asia/Bangkok');
                $cutoffTime = $timeSlot->order_cutoff_time;
                $basePlusDays = (int)($timeSlot->deli_plus_days ?? 0);
                $startMinutes = (int)($timeSlot->seller_start_deli_time ?? 0);
                $endMinutes = (int)($timeSlot->seller_end_deli_time ?? 0);
                ...
                // 2. ตั้งฐานเวลาจาก Cutoff Time
                $cutoffCarbon = Carbon::parse($cutoffTime);
                $baseDateTime = $now->copy()->startOfDay()
                                    ->addDays($basePlusDays)
                                    ->addHours($cutoffCarbon->hour)
                                    ->addMinutes($cutoffCarbon->minute);

                // 3. คำนวณเวลาเริ่ม
                $startTimeObj = $baseDateTime->copy()->addMinutes($startMinutes);
                $dc_delivery_starttime = $startTimeObj->toDateTimeString();

                // 4. คำนวณเวลาสิ้นสุด (จัดการเรื่องข้ามวัน)
                // ถ้า end < start แสดงว่าต้องบวกไปอีก 1 วัน (1440 นาที)
                if ($endMinutes < $startMinutes) {
                    $endTimeObj = $baseDateTime->copy()->addMinutes($endMinutes + 1440);
                } else {
                    $endTimeObj = $baseDateTime->copy()->addMinutes($endMinutes);
                }
                $dc_delivery_endtime = $endTimeObj->toDateTimeString();

                // Log ดูความสมเหตุสมผล
                \Log::info("Start: $dc_delivery_starttime | End: $dc_delivery_endtime");
            }

            $main_order->del_t_s_id = $orderInfo->del_t_s_id;
            $main_order->end_pickup_time = $orderInfo->end_pickup_time; 
            // $main_order->dc_delivery_starttime = $dc_delivery_starttime; // อย่าลืม assign ค่าถ้าต้องการ save ลง db
            // $main_order->dc_delivery_endtime = $dc_delivery_endtime;
            $main_order->save();
        }
        ...
        if (!empty($shop_id_arr)) {
            ...
            foreach ($shop_id_arr as $skey => $svalue) {
                ...
                $shop_order = OrderShop::createShopOrder($svalue,$main_order,$user,$shop_detail_arr[$svalue],$order_json, $dc_delivery_starttime, $dc_delivery_endtime);
    ...
    public static function saveOrderEndShopping($orderId,$main_order,$temp_ord_info=null) {
        ...
        $main_order->end_pickup_time      = $temp_ord_info->end_pickup_time;
        ...
        $main_order->del_t_s_id    = $temp_ord_info->del_t_s_id;
        $main_order->end_pickup_time    = $temp_ord_info->end_pickup_time;
        $main_order->save();
