...
class CartController extends MarketPlace {
	...
	public function index(Request $request) {
		try {
			...
			return view('checkout.cart',compact(
				...
				[
					'orderInfo' => $orderInfo, 'orderDetails'=>$orderDetails,'page_class'=>'cart-wrap',
					'breadcrumb'=>$breadcrumb,'shop_address'=>$shop_address,
					'main_order'=>$main_order,'paid_product'=>$paid_product,
					'shipping_fee'=>$shipping_fee,'user_odd_info'=>$user_odd_info,
					'time_arr'=>[],'delivery_time_arr'=>$delivery_time_arr,'transaction_fees'=>$transaction_fees
				]
			);
	...
	function saveAddress(Request $request) {
		...
		if (!$validate->fails()) {
			...
			$str_common = '<p>'.$request->first_name.' '.$request->last_name.'</p><p>'.$request->address.', '.$request->road.'</p><p>'.$address->city_district.', '.$address->province_state.' '.$request->zip_code.'</p><p>'.Lang::get("customer.tel").' : '.$request->ph_number.'</p>';
