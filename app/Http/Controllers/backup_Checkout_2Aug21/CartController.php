...
class CartController extends MarketPlace {
	...
	public function index(Request $request) {
		...
		***/
		...
		if($checkout_type == 'end-shopping' || $checkout_type == 'buy-now-end-shopping'){
	        ...
	        if(getConfigValue('ADDRESS_TYPE') == 'dropdown' && count($def_country_dtl) > 0) {
	...
	protected function getCalculateProductsShipFee($items,$shipping_address,$shipProfileData){
		...
		$shipping_profile_id = $shipProfileData->id;
		$shipping_rate_data = DB::table(with(new \App\ShippingProfileRates)->getTable().' as spr')
		->leftJoin(with(new \App\ShippingProfileRatesDesc)->getTable().' as sprd', 'spr.id', '=', 'sprd.rate_id')->select('spr.id', 'spr.country_id', 'sprd.province_state','sprd.district_city', 'sub_district', 'spr.weight_from', 'spr.weight_to', 'spr.qty_from', 'spr.qty_to', 'spr.price_from', 'spr.price_to', 'spr.zip_from', 'spr.zip_to', 'spr.product_type_id','spr.estimate_shipping','spr.base_rate_for_order','spr.percentage_rate_per_product','spr.fixed_rate_per_product','spr.fixed_rate_per_unit_weight','spr.logistic_base_rate_for_order','spr.logistic_percentage_rate_per_product','spr.logistic_fixed_rate_per_product','spr.logistic_fixed_rate_per_unit_weight','spr.priority')->whereIn('spr.country_id',[$shipping_address->country_id,$astric])
							->whereIn('sprd.province_state',[$shipping_address->province_state,$astric])
							->whereIn('sprd.district_city',[$shipping_address->city_district,$astric])
							->whereIn('sprd.sub_district',[$shipping_address->sub_district,$astric])
							->where(['sprd.lang_id'=>session('default_lang'),'spr.shipping_profile_id'=>$shipping_profile_id])
	...
	function store(Request $request){
		...
		if ($validate->passes()) {
			...
			if($payment_slug == 'kbank'){
				$url = action('Checkout\CartController@kbankPayment');
				return ['status'=>'success','url'=>$url];
			}
			if($payment_slug == 'payplus'){
				$url = action('Checkout\CartController@payplusPayment');
				return ['status'=>'success','url'=>$url];
			}
			if($payment_slug == 'odd'){
				$return = $this->oddPayment($orderInfo->id,$user_odd_info);
				return $return;
			}
			try{
	...
	function kbankPayment(Request $request){
		$userid = Auth::User()->id;
		$orderInfo = OrdersTemp::where(['user_id'=>$userid,'order_status'=>'0'])->first();
		...
		if($order){
			return view('checkout.kbank' ,['orderInfo' => $orderInfo, 'kbank_details'=>$kbank_details,'order_id'=>$order]); 
		}else{
			abort(404);
		}
	...
	function payplusPayment(Request $request){
		$userid = Auth::User()->id;
		$orderInfo = OrdersTemp::where(['user_id'=>$userid,'order_status'=>'0'])->first();
		...
		if(count($orderInfo)){
			...
			if($orderInfo->payment_slug == 'payplus'){
				return view('checkout.payplus' ,['orderInfo' => $orderInfo]);
			}
		}else{
			abort(404);
		}
	...
	function createPayPlusOrder(Request $request){
        ...
        $update_ord = OrdersTemp::where('id',$orderInfo->id)->update(['kbank_qrcode_id'=>$invoice]);

        echo base64_encode(json_encode($response_array));

	}

	function oddPayment($order_id,$user_odd_info){
		$userid = Auth::User()->id;
		
		$temp_ord_info = OrdersTemp::getTempOrderInfo($order_id);

		$userDetail = Auth::user();

		$amount = number_format($temp_ord_info->total_final_price,2);
		$amount = "$amount";
        $ref_no = generateUniqueNo();

        $update_ord = OrdersTemp::where('id',$order_id)->update(['kbank_qrcode_id'=>$ref_no]);
        	...
        	if($response &&  isset($response['transaction_list'][0]['return_status']) && $response['transaction_list'][0]['return_status']=='0'){
        		...
        		if(count($temp_ord_info)){
	                ...
	                if($temp_ord_info->checkout_type == 'buy-now-end-shopping' || $temp_ord_info->checkout_type == 'end-shopping'){
	                    ...
	                    $main_order = \App\Order::where('temp_formatted_id',$temp_ord_info->formatted_order_id)->first();
	                    if(count($main_order)){
	                        ...
	                        if($main_order->total_shipping_cost > 0){
	                            ...
	                            $ref_ord = $response['transaction_list'][0]['external_reference']??'';
	                            $arr = ['order_id'=>$main_order->id,'payment_slug'=>'odd','reference_order'=>$ref_ord,'items'=>'shipping','response'=>json_encode($response),'created_at'=>$current_date];
	                            $update_pay_resp = \App\OrderPayment::insert($arr);
	                        }

	                        return ['status'=>'success','url'=>action('Checkout\OrderController@thanks',$main_order->formatted_id)];
	                    }
