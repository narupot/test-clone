...
class OrderController extends MarketPlace {
    ...
    public static function saveFinalOrder($orderId){
        ...
        $shippingAddressArr = $billingAddressArr = $shipping_profile_arr = [];

        
        $orderJson = json_encode(['shipping_address'=>$shippingAddressArr,'billing_address'=>$billingAddressArr,'shipping_profile'=>$shipping_profile_arr]);
        
        $cartInfo = Cart::where('order_id',$orderInfo->id)->with('getPrd')->get();
        ...
        $Orders->total_core_cost      = $orderInfo->total_core_cost;
        $Orders->total_discount       = $orderInfo->total_discount;
        $Orders->total_shipping_cost  = $orderInfo->total_shipping_cost;
        $Orders->total_final_price    = $orderInfo->total_final_price; 
        $Orders->end_shopping_date    = $current_date;
        ...
        /***entry into order shop*********/
        foreach ($shop_id_arr as $skey => $svalue) {
            
            $shop_obj = new OrderShop;
            $shop_obj->order_id = $main_order_id;
            $shop_obj->shop_id = $svalue;
            $shop_obj->user_id = $orderInfo->user_id;
            $shop_obj->user_name = $user_name;
            $shop_obj->user_email = $user_email;
            $shop_obj->end_shopping_date = $current_date;
            $shop_obj->save();

            $shop_order_id = $shop_obj->id;
            $formatted_shop_id = OrderShop::formattedOrder().$shop_order_id;
                ...
                foreach ($shop_cart_detail[$svalue] as $key => $detail) {
                    ...
                    if(!empty($detail->getPrd)){
                        ...
                        if($detail->getPrd->stock == 0){
                            $cart_qty = $detail->quantity;
                            $prd_qty  = $detail->getPrd->quantity;
                            $remain_qty = $prd_qty - $cart_qty;

                            $update_qty = \App\Product::where('id',$detail->product_id)->update(['quantity'=>$remain_qty]);
                        }
                    ...
                    $order_detail->payment_date     = $current_date;
                    $order_detail->order_detail_json = jsonEncode($product_detail_arr);

                    $order_detail->save();

                    $shop_total_price = $shop_total_price + $detail->total_price;

                    if($orderInfo->payment_type == 'credit'){
                        $total_credit_amount = $total_credit_amount + $detail->total_price;
                        $credit_due_date = addDaysTodate($current_date,15);
                    }
                }

                /***updating shop total price and credit price*****/
                $update_shop_ord_amt = OrderShop::where('id',$shop_order_id)->update(['shop_formatted_id'=>$formatted_shop_id,'total_core_cost'=>$shop_total_price,'total_final_price'=>$shop_total_price,'total_credit_amount'=>$total_credit_amount,'credit_due_date'=>$credit_due_date]);
            }
        }

        /***delete/release hold qty***/
        $deleteHold = \App\OrderQuantityHold::deleteHoldQty($orderId);

        /**cteating order transaction*******/
        if($orderInfo->payment_type == '2'){ /**offline**/
            $transArr = ['order_id'=>$main_order_id,'status_id'=>4,'event'=>'payment','updated_by_id'=>$orderInfo->user_id,'updated_by'=>'buyer','comment'=>'Order created offline','created_at'=>date("Y-m-d H:i:s")];
        }else{
            $transArr = ['order_id'=>$main_order_id,'status_id'=>2,'event'=>'payment','updated_by_id'=>$orderInfo->user_id,'updated_by'=>'buyer','comment'=>'Order created online','created_at'=>date("Y-m-d H:i:s")];
        }

        //$updateOrderTrans = Orders::updateOrderTrans($transArr);
        dd($formattedOrderId);
    ...
    public function thanks(Request $request){
        ...
        if(count($main_order)){
            ...
            $shop_ord = \App\OrderShop::where('order_id',$main_order->id)->select('id','shop_formatted_id')->get();
            if(count($shop_ord)){
    ...
    public static function getAddressArr($addInfo) {
        ...
        $AddressArr = ['first_name'=>$addInfo->first_name,'last_name'=>$addInfo->last_name,'provice'=>$addInfo->province_state,'district'=>$addInfo->city_district,'address'=>$addInfo->address,'road'=>$addInfo->road,'zip_code'=>$addInfo->zip_code,'ph_number'=>$addInfo->ph_number,'company_name'=>$addInfo->company_name,'branch'=>$addInfo->branch,'tax_id'=>$addInfo->tax_id,'company_address'=>$addInfo->company_address];
    ...
    public static function getShopArr($shop_detail){
        ...
        if(count($shop_detail)){
            ...
            $arr = ['shop_url'=>$shop_detail->shop_url,'panel_no'=>$shop_detail->panel_no,'market'=>$shop_detail->market,'logo'=>$shop_detail->logo,'shop_name'=>$name_arr,'ph_number'=>$shop_detail->ph_number];
    ...
    public static function createKbankOrder($orderInfo,$kbank_details){
        ...
        if(!empty($response->id)){
            $update_ord = OrdersTemp::where('id',$orderInfo->id)->update(['kbank_qrcode_id'=>$response->id]);
            return $response->id;
        }else{
            return 0;
        }
