    ...
    private function prepareShippingNoteData($main_order, $json_arr, $end_shopping_date)
    {
        ...
        if(count($order_shop)){
            foreach ($order_shop as $shop_key => $shop_value) {
                ...
                $order_detail = \App\OrderDetail::query()
                    ->select('od.*', DB::raw('smm_pw.avg_revised_weight'))
                    ->from('order_detail as od')
                    ->leftJoin('wh_master_product_weights as pw', function ($join) {
                        $join->on('od.category_name', '=', 'pw.category_name')
                            ->on('od.package_name', '=', 'pw.package_title')
                            ->on('od.base_unit', '=', 'pw.base_unit_title')
                            ->on('od.total_weight', '=', 'pw.weight_per_unit');
                    })
                    ->where('od.order_shop_id', $shop_value['id'])
                    ->get()
                    ->toArray();
                ...
                // $order_detail = \App\OrderDetail::query()
                //     ->from(DB::raw('smm_order_detail as smm_od'))
                //     ->leftJoin(DB::raw('smm_wh_master_product_weights as smm_pw'), function ($join) {
                //         $join->on('od.category_name', '=', 'pw.category_name')
                //             ->on('od.package_name', '=', 'pw.package_title')
                //             ->on('od.base_unit', '=', 'pw.base_unit_title')
                //             ->on('od.total_weight', '=', 'pw.weight_per_unit');
                ...
                foreach ($order_detail as $dkey => $dvalue) {
                    ...
                    $item_weight_per_unit = !empty($dvalue['avg_revised_weight']) 
                                    ? $dvalue['avg_revised_weight'] 
                                    : $dvalue['total_weight'];

                    if(empty($dvalue['avg_revised_weight']) || $dvalue['avg_revised_weight'] <= 0) {
