<header id="header" style="position:sticky; top:0; z-index:3; background-color:white; ">
    <!--<header id="header">-->
    <div class="container">
        <div class="header-inner">
            <div class="logo">
                <a href="{{ action('HomeController@index') }}" title="Smmarket"><img src="{{ getSiteLogo('SITE_LOGO_HEADER') }}" width="70%" max-width="125px" height="auto" alt=""></a>
            </div>
            <div class="header-content">
                <div class="login-link">
                    <ul>
                        @if(Auth::check())
                            <!--<li><a href="{{action('User\UserController@index')}}"><img class="account-icon" src="{{ getUserImageUrl(Auth::user()->image) }}" alt="">{{ Auth::user()->display_name }}</a></li>-->
                ...
                <div class="search-cart-wrap">
                    ...
                    @if(Auth::check())
                        ...
                        @php $tot_order_pending = getPendingOrderNoti(); @endphp
                        <div class="head-col buyer-col chat-Wrap @if(strpos( $_SERVER['REQUEST_URI'], 'seller' ) !== false) mobhide @endif">
                            <!--<div class="buyer-pannel">
                                <a href="#" class="btn-grey dropdown-toggle" data-flip="false" data-toggle="dropdown"><i class="badge" style="display: {{ $tot_cart_prd_noti['tot']>0?'inline-block':'none' }};" id="tot_cart_noti">{{ $tot_cart_prd_noti['tot']}}</i> !--before 240927 start <i class="fas fa-shopping-basket"></i> before 240927 stop-- <img src="images/acoount-icon.png" height="22" alt="img"> @lang('checkout.buyer_panel')</a>
                            ...
                            <span class="chat-wrap">
                                <a class="dropdown-item" href="{{ action('Checkout\CartController@shoppingCart') }}">
                                    <i class="fas fa-shopping-basket" style="display: block; text-align: center;"></i>
                                    <i class="badge" id="tot_prd_noti" style="display: {{ $tot_cart_prd_noti['cart_prd']>0?'inline-block':'none' }};">
                                        {{ $tot_cart_prd_noti['cart_prd'] }}</i>
                                     @lang('checkout.cart_for_payment') 
                                    </a>
                            </span>
                            <span class="chat-wrap">
                                <a href="{{ action('User\OrderController@pendingOrder') }}" class="@if(strpos( $_SERVER['REQUEST_URI'], 'user/order/pending-order' ) !== false && strpos( $_SERVER['REQUEST_URI'], 'delivery-list' )== false) active @endif">
                                    <i class="fas mr-2" style="display: block; text-align: center;">
                                        <img src="images/pendingOrder.jpg" alt="img" height="18px" class="rounded-0" >
                                    </i>
                                    <i class="badge" id="tot_pending_order_noti" style="display: {{ $tot_order_pending>0?'inline-block':'none' }};">
                                        {{ $tot_cart_prd_noti['cart_prd'] }}</i>
                                    @lang('order.pending_order_N')</a>
                            </span>
                            <!--<span class="chat-wrap">
                                <a class="dropdown-item" href="user/order/history">
                                    <i class="fas mr-2" style="display: block; text-align: center;">
                                    <img src="images/order.png" alt="img" height="18px" class="rounded-0" > 
                                    </i>
                                    @lang('customer.order_history_N')</a>
                            </span>-->
                            <span class="chat-wrap btn-buyer-chat" data-val="">
                                <a href="javascript:void(0);"><i class="fas fa-comments mr-2" style="display: block; text-align: center;"></i>@lang('product.talk_to_shop_home')</a>
                            </span>
