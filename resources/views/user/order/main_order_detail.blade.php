...
<div class="track-order-num-wrap border-bottom pb-2">
    ...
    <div class="d-flex mb-3 mt-3 justify-content-between flex-wrap">
        <div class="track-status form-group mb-2">
            ...
            <span class="ship-track-time d-block mt-2 col">{{getDateFormat($main_order->updated_at,11)}}</span>
        </div>
...
<div class="title-track-info">
    ...
    <div class="track-ship-name">{{ GeneralFunctions::getShippingMethod($main_order->shipping_method) }} @if(strtotime($main_order->pickup_time)) <span class="red">(@lang('order.expected_time_to_receive') : {{ getDateFormat($main_order->pickup_time,11) }} @if(isset($main_order->plus_two_hr)) - {{$main_order->plus_two_hr}}:00 @endif)</span> @endif</div>
</div>
