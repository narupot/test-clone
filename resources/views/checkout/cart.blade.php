...
@section('header_script')
    ...
    var address_dd_url = "{{ (!empty($def_country_dtl) && $def_country_dtl->short_code == 'TH') ? route('smm.address.dd') : action('AjaxController@getStateCityDD') }}";
    var use_smm_address = {{ (!empty($def_country_dtl) && $def_country_dtl->short_code == 'TH') ? 'true' : 'false' }};
                        ...
                        <div class="ship-method ">
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane col" id="pick_up_center">
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                                <address class="post-address" id="ship-info"> 
                                        </div>
                                    </div>
                                
                                </div>
                            </div>
                            <div class="row" id="user_phone_no_div">
                                <div class="col-md-3 mb-3">
                                    <div class="form-group">
                                        <label for="phone_no">@lang('checkout.phone_no') <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="phone_no" id="phone_no" placeholder="เบอร์โทรศัพท์ผู้รับ">
                                        <p class="error text-danger mt-1" id="e_phone_no"></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    <div class="form-group">
                                        <label for="pickup_time_n">@lang('checkout.pickup_time') <span class="text-danger">*</span></label>
                                        
                                        <select class="form-control" name="pickup_time" id="pickup_time_n">
                                            <option value="">@lang('checkout.select_pickup_time')</option>
                                            
                                            {{-- ข้อมูลเริ่มต้น (ถ้ามี) --}}
                                            @if(isset($time_slots_initial))
                                                @foreach($time_slots_initial as $slot)
                                                    <option value="{{ $slot->del_t_s_id }}">
                                                        {{-- ใส่ Logic การแสดงผล Text ตามที่คุณมี --}}
                                                        {{ $slot->start_deli_time }} - {{ $slot->end_deli_time }}
                                                    </option>
                                                @endforeach
                                            @endif
                                        </select>

                                        <p class="error text-danger mt-1" id="e_pickup_time"></p>
                                        <input type="hidden" name="nexday" id="nexday" value="{{ $delivery_details['tomorrow'] ?? '' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                success:function(result){
                    ...
                    if(result.status == 'success'){
                        ...
                        let message = "คุณได้รับส่วนลด" +
                            (data?.discountPurchase > 0 ? ` ${formatMoney(data.discountPurchase)} บาท` : '') +
                            (data?.discountShipping > 0 ? ` ค่าจัดส่ง ${formatMoney(data.discountShipping)} บาท` : '');
                        swal({
                            title: message,
                            type: 'success',
