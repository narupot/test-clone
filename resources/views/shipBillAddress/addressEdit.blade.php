<style type="text/css">
    ...
    /* สไตล์ส่วนพิกัดแผนที่ - กล่องสีเขียวอ่อนกรอบประ */
    .coordinate-box {
        padding: 12px 16px;
        border-radius: 6px;
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="font-weight-bold">@lang('customer.edit_address')</h3>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6">
                            ...
                            <select class="address_dd" name="province_state" id="address_dd_1" address_seq="1">
                                @if(isset($use_smm_address) && $use_smm_address)
                                    {!! CustomHelpers::getSmmProvinceDD($address->province_state_id) !!}
                                @else
                                    {!! CustomHelpers::getProvinceStateNormalDD($def_country_dtl->id ?? 0, $address->province_state_id) !!}
                                @endif
                            </select>
                        ...
                        <div class="col-sm-6">
                            ...
                            <select class="address_dd" name="city_district" id="address_dd_2" address_seq="2">
                                @if(isset($use_smm_address) && $use_smm_address)
                                    {!! CustomHelpers::getSmmCityDistrictDD($address->province_state_id, $address->city_district_id) !!}
                                @else
                    ...
                    <div class="form-group row" id="sub_district_row">
                        <div class="col-sm-6">
                            ...
                            <select class="address_dd" name="sub_district" id="address_dd_3" address_seq="3">
                                {!! CustomHelpers::getSmmSubDistrictDD($address->city_district_id, $address->sub_district_id) !!}
                            </select>
                            <span id="error_sub_district" class="error-msg"></span>
                        </div>
                        <div class="col-sm-6">
                            <label>@lang('common.zip')<i class="red">*</i></label>
                            <input type="text" name="zip_code" id="zip_code" value="{{ $address->zip_code }}" readonly>
...
<script type="text/javascript">
    if (typeof map_picker_url === 'undefined') {
        map_picker_url = '{{ route("user.address.map-picker") }}';
    }
    ...
    function openMapPicker() {
        // โหมดแก้ไข: โหลดแผนที่แยก (ไม่ใช้ map-picker-modal ของ addressList ที่ผูกกับฟอร์มเพิ่ม)
        var isEditMode = $('#addess_frm input[name="address_id"]').length && $('#addess_frm input[name="address_id"]').val();
        if ($('#map-picker-modal').length && !isEditMode) {
            ...
            var params = {
                call_type: 'ajax_data',
                lat: $('#lat').val() || 13.9562464,
                long: $('#long').val() || 100.6154008,
