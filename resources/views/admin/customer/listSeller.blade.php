        ...
        <div class="content-wrap">
            <div class="breadcrumb">
                <ul class="bredcrumb-menu">
                    {!!getBreadcrumbAdmin('seller', 'seller', 'list')!!}
                </ul>
            </div> 
...
@section('footer_scripts')
    ...
    <script>
        let view_url = "{{action('Admin\Customer\SellerController@index')}}";
        let JQ_GRID_DATA_URL = "{{ action('Admin\Customer\SellerController@sellerData') }}";
        var BATCH_ACTION_DELETE = {
            action_name : "@lang('admin_common.delete')",
            action_url : "{{action('Admin\Customer\SellerController@deleteSelectedSeller')}}",
            btn_class : 'btn',
        };
        var BATCH_ACTION_STATUS = {
            action_name : "@lang('admin_common.change_status')",
            action_url : "{{action('Admin\Customer\SellerController@changeStatusofSelectedSeller')}}",
            btn_class : 'btn',
            action_options : {
                1 : 'Active',
                0 : 'InActive',
        ...
        const JQ_GRID_TITLE = "@lang('admin_customer.customer_seller_list')";       
        /*
        ...
        */
        ...
        let columnModel = [  
            ...
            {   title: "@lang('admin_customer.ph_number')",
                dataIndx:'ph_number', 
                minWidth: 140,
                menuIcon : !1,
                filter: {
            ...
            {   title: "@lang('admin_customer.shop_name')", 
                dataIndx:'shop_name',
                minWidth: 140,
                menuIcon : !2,
                filter : {
            ...
            {   title: "@lang('admin_customer.shop_url')", 
                dataIndx:'shop_url',
                minWidth: 140,
                menuIcon : !3,
                filter : {
                    attr : "@lang('admin_common.enter_shop_url')",                        
                    crules: [
                        {
                            condition: getFilter('shop_url', 'condition') ||  'contain',
                            value : getFilter('shop_url', 'value')  || "",
                        }
                    ],
                    type: 'textbox', 
                    listeners: ['change'],
                ...
                filter : {
                    ...
                    crules: [
                        {
                            condition: getFilter('panel_no', 'condition') ||  'contain',
                            value : getFilter('panel_no', 'value')  || "",
                        }
                    ],
                    type: 'textbox', 
                    listeners: ['change'],
