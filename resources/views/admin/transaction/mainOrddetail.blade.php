                                            ...
                                            <div class="order-status-content m-0 px-0" id="change_pickup_time" style="display:none;">
                                                <div class="col-sm-12 d-flex">
                                                    ...
                                                    <input type="date" 
                                                        ...
                                                        style="width: 180px;"  
                                                        value="{{ $main_order->pickup_time ? explode(' ', $main_order->pickup_time)[0] : now()->addDay()->format('Y-m-d') }}">
                                                                                                        
                                                    <!-- เลือกรอบเวลา -->
                                                    @php
                                                        $pickup_time_only = date('G:i', strtotime($main_order->pickup_time)); // แปลง "2025-08-29 09:00:00" เป็น "9:00"
                                                    @endphp

                                                    <select style="width: 170px;" class="form-control mr-2" name="pickup_time" id="pickup_time_id">
                                                        @foreach($pickup_time_arr as $val)
                                                            <option value="{{ $val['key'] }}" {{ $val['val'] == $pickup_time_only ? 'selected' : '' }}>
                                                                {{ $val['val'] }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <!-- <a href="javascript::void(0);" id="update_pickup_time" class="btn btn-primary mt-2">
                                                    @lang('admin_common.update')
                                                </a> -->
                                                <button type="button" id="update_pickup_time" class="btn btn-primary mt-2">
                                                    @lang('admin_common.update')
                                                </button>
                                            </div>
...
<script type="text/javascript">
    ...
    $('body').on('click', '#update_pickup_time', function() {
        var pickup_date = $('#pickup_date_id').val();
        var pickup_time = $('#pickup_time_id').val();
        var order_id = {{ $main_order->id }};

        if(!pickup_date || !pickup_time){
            $('#e_pickup_time').text('กรุณาเลือกวันและเวลา');
            return;
        }

        var pickup_datetime = pickup_date + ' ' + pickup_time;

        Swal.fire({
            text: "ยืนยันเปลี่ยนรอบการจัดส่งเป็น " + pickup_datetime + "?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'ยืนยัน',
            cancelButtonText: 'ยกเลิก'
            ...
            if(result.isConfirmed){
                ...
                var data = {order_id: order_id, pickup_time_id: pickup_datetime};

                callAjax(ajax_url, 'post', data, function(res) {
                    Swal.fire('', res.msg, res.status).then(function() {
                        location.reload();
                    });
                });
            }
