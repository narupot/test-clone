                                    ...
                                    @foreach($order_shop->details as $key => $val)
                                        ...
                                        <ul>
                                            ...
                                            <li class="add-rem-qty">
                                                ...
                                                <input type="text" id="txt_item_qty{{$val->id}}" name="item_quantity" value="{{ $val->quantity }}">
                                            ...
                                            @if(!$order_shop->end_shopping_date || $order_shop->order_status ==3 || $order_shop->order_status ==4)
                                            <li>@if($val->status!=4) <a href="javascript:;" data-type='cancel' data-val="{{ $val->id }}" class="ord_item_change">@lang('common.cancel')</a> @endif | <a href="javascript::void(0);" onclick="$('#update_item_quantity{{$val->id}}').show();$(this).hide();" id="change_update_item_quantity{{$val->id}}">แก้ไขจำนวนสินค้า</a></li>
                                            @else
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('body').on('click', '.button_update_item_quantity', function() {
            ...
            swal({
                text: lang_update_item_quantity + ' ' + order_detail_id + ' ' + change_item_quantity,
                type: 'warning',
