...
<div class="container-fluid" style="max-width: 1200px; margin: auto;">
    ...
    <form method="GET" class="row g-3 mb-4">
        ...
        <div class="col-md-3">
            <label for="filter" class="form-label">ตัวกรอง</label>
            <select id="filter" name="filter" class="form-select" onchange="this.form.submit()">
                <option value="all" {{ (isset($filter) && $filter=='all')? 'selected':'' }}>พิมพ์ทั้งหมด</option>
                <option value="transaction_fee" {{ (isset($filter) && $filter=='transaction_fee')? 'selected':'' }}>รายการที่มีค่าธรรมเนียมการโอน</option>
                <option value="shipping" {{ (isset($filter) && $filter=='shipping')? 'selected':'' }}>รายการที่มีค่าขนส่ง</option>
            </select>
        </div>
        <div class="col-md-3">
            <label>&nbsp;</label>
            <button type="button"
                    onclick="window.open('{{ route('admin.order.shipping-receipt-bulk', ['delivery_date' => $deliveryDate, 'filter' => $filter ?? 'all']) }}', '_blank')"
