...
@section('content')
    <div class="content">
        <form id="campaignForm" action="{{ route('admin.campaign.megaCampaign.update',['campaign'=>$campaign->id]) }}" method="post" 
            class="form-horizontal form-bordered" novalidate="novalidate" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}        
            <div class="header-title">
                ...
                @endif 
                <h1 class="title">Edit MEGA Campaign  {{$badge_dtls->badgedesc->badge_name ?? ''}}</h1> 
                <div class="float-right">                
                    <a class="btn btn-back" href="{{ action('Admin\Campaign\CampaignController@indexMegaCampaign') }}">@lang('common.back')</a>
