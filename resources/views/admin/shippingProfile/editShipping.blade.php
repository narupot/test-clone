...
<script type="text/javascript">
    ...
    var selectedCountry = {!! $selected_country !!};
    var provinceData = [];
    var selProvinceInd = [];
    var angular_state = {!! $angular_state !!};
...
@section('content')
    <div class="content ng-cloak" ng-controller="shipProfileCtrl" ng-cloak>
        ...
        @endif 
        <form id="cmsForm" action="{{ action('Admin\ShippingProfile\ShippingProfileController@update',$shipping_details->id) }}" method="post" class="form-horizontal form-bordered">
            <div class="header-title">
                <h1 class="title">@lang('admin_shipping.edit_shipping_profile') : {{$shipping_details->shipping_key}}</h1>
                <div class="float-right">
                    <a href="{{ action('Admin\ShippingProfile\ShippingProfileController@index') }}" class="btn btn-back">@lang('admin_common.back')</a>
                    <button name="submit_type" value="save" class="btn btn-primary" type="submit">@lang('admin_common.update')</button>
                </div>
            </div>       
            <div class="content-wrap">
                ...
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                {!! Form::hidden('ship_id', old('shipping_id', $shipping_details->id), ['id'=>'ship_id']) !!}
                <div class="form-group">
                    <div class="col-md-6">
                    <label for="form-text-input">@lang('admin_shipping.shipping_key') <i class="strick">*</i></label> 
                        <input type="text" name="shipping_key" value="{{$shipping_details->shipping_key }}">
                ...
                <div class="form-group">
                    ...
                    {!! CustomHelpers::fieldstabWithLanuageEdit([['field'=>'text', 'name'=>'name', 'label'=>' ', 'errorkey'=>'ship_name']], '1', 'shipping_profile_id', $shipping_details->id, $tableName, $errors) !!}
                ...
                <div class="form-group">
                    ...
                    <label for="form-text-input">@lang('admin_shipping.type')</label>
                     {!! Form::select('shippingType', ['flat-rate'=>Lang::get('shipping.flat_rate'), 'free-shipping'=>Lang::get('shipping.free_shipping')],   $shipping_details->shipping_type,['id'=>'shippingType']) !!}
                    ...
                    <div class="condition-rulebox">
                            ...
                            <label>@lang('admin_shipping.use_with_promo')</label>
                                <div class="radio-group">
                                    <label class="radio-wrap">
                                        <input type="radio" name="promo" value="1" @if($shipping_details->use_promo==1) checked @endif>
                                        <span class="radio-label "> @lang('admin_shipping.yes')</span></label>
                                        <label class="radio-wrap">
                                        <input type="radio" name="promo" value="0" @if($shipping_details->use_promo==0) checked @endif>
                                        <span class="radio-label ">@lang('admin_shipping.no') </span></label>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row flat-row-field">
                            ...
                            <label>@lang('admin_shipping.type')</label>
                                  {!! Form::select('flat_rate_type', ['per-item'=>Lang::get('shipping.per_item')],  $shipping_details->flat_type,[ 'class'=>'custom-select']) !!}
                            </div>
                        </div>                
                        <div class="row form-group flat-row-field">
                            <div class="col-sm-6">
                            <label>@lang('admin_shipping.shipping_fee')</label>
                                <input type="text" value="{{old('shipping_fee',$shipping_details->flat_shipping_fee)}}" name="shipping_fee"> 
                        ...
                        <div class="row form-group free-ship-field">
                            ...
                            <label>@lang('admin_shipping.minimum_order_amount')</label>
                                {!! Form::text('freeShipamt', old('freeShipamt',$shipping_details->free_order_amount)) !!}
                                 @if ($errors->has('freeShipamt'))
                                <p id="comment-error" class="error error-msg">{{ $errors->first('freeShipamt') }}</p>
                                @endif
                            </div>                              
                        </div>                          
                        <div class="form-group row search-select" id="specific_country">
                            <div class="col-sm-9">  
                        ...
                        <div class="row form-group flat-row-field">
                            ...
                            <label style="margin-bottom: 5px;">@lang('admin_shipping.rest_of_uncheck_country_action')</label>
                                <label class="radio-wrap"><input type="radio" name="rest_country" class="rest_country" value="1" @if($shipping_details->rest_country==1) checked @endif>  <span class="radio-label ">@lang('admin_shipping.set_delivery_for_the_rest_of_the_country')</span></label>
                                <div class="delv-input form-group">
                                    <div class="rest_country_opt" @if($shipping_details->rest_country==0) style="display:none" @endif>
                                        <input type="text" class="rest_country_opt" name="rest_country_price" style="width: 24%" value="{{$shipping_details->rest_country_price}}">
                                        @if ($errors->has('rest_country_price'))
                                        <p id="comment-error" class="error error-msg">{{ $errors->first('rest_country_price') }}</p>
                                        @endif
                                        <select style="width: 50%;" class="rest_country_opt" name="rest_country_type">
                                            <option value="per-item" @if($shipping_details->rest_country_type=='per-item') selected="selected" @endif>@lang('admin_shipping.per_item')</option>
                                            <option value="per-order" @if($shipping_details->rest_country_type=='per-order') selected="selected" @endif>@lang('admin_shipping.per_order')</option>
                                        </select>
                                    </div>
                                </div>
                                <label class="radio-wrap"><input type="radio" name="rest_country" class="rest_country" value="0"  @if($shipping_details->rest_country==0) checked @endif>
                       ...
                       {{-- <div class="row flat-row-field">
                            ...
                            <div class="form-group">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                      <select class=""  multiple options="provinceData" ng-model="selectedProvince" ng-options="province.name for province in provinceData track by province.id" name="shipping_province[]">
                                      </select>
                                    </div>
                                    <label class="check-wrap"><input type="checkbox" name="province_mode" value="1" @if($shipping_details->province_mode==1) checked @endif> <span class="chk-label">@lang('admin_shipping.include_mode_(if_checked,_list_above_will_be_delivery_only)')</span></label>
                                </div>                                
                            </div>                        
                        </div> --}}
                        <div class="row form-group flat-row-field">
                            ...
                            <div class="form-group" ng-show="country_manage.show_state== true && provinceData.length"> 
                                ...
                                <div class="col-sm-6">             
                                    <div class="form-group custom-width">
                                        <select multiple options="provinceData" ng-model="selectedProvince" class="select" name="shipping_province[]" ng-change="provinceChange()">
                        ...
                        <div class="row flat-row-field">
                            ...
                            <div class="form-group">
                                <div class="col-sm-6">
                                    <div class="form-group custom-width">
                                        <select class="select" chosen multiple name="shipping_product[]">
                                            
                                            @foreach($productList as $productRes)
                                            <option value="{{$productRes['id']}}"  @if(in_array($productRes['id'],$selected_product)) selected @endif>{{$productRes['url']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <label class="check-wrap"><input type="checkbox" name="product_mode" value="1" @if($shipping_details->product_mode==1) checked @endif> <span class="chk-label">@lang('admin_shipping.include_mode_(if_checked,_list_above_will_be_delivery_only)')</span></label>
                                </div>
...
<script type="text/javascript">
    ...
    jQuery('body').on('change','.rest_country',function(event){
      var rVal = $(this).val();
      if(rVal == 1){
    ...
    jQuery('body').on('change','#country_type',function(){
    var val = $(this).val();
    if(val == 'all'){
