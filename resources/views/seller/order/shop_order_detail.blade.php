    ...
    <div class="container1">
        <div class="track-order-num-wrap border-bottom pb-2">
            <div class="cust-ord-num">
                <div class="float-right btn-group">
                    <a href="{{ $previous_url ?? 'javacript:;' }}" class="btn-grey"><i>
                            < </i> @lang('common.back')</a>
        ...
        <div class="title-track-info">
            ...
            <div class="track-ship-name">
                {{ GeneralFunctions::getShippingMethod($orderShopData->shipping_method) }}

                @if (strtotime($orderShopData->dc_delivery_starttime) > 0)
                    <span class="red">
                        (@lang('order.expected_time_to_sending') :

                        @php
                            $start = $orderShopData->dc_delivery_starttime;
                            $end = $orderShopData->dc_delivery_endtime;

                            $isSameDay = date('Y-m-d', strtotime($start)) === date('Y-m-d', strtotime($end));
                        @endphp

                        @if ($isSameDay)
                            {{ getDateFormat($start, 11) }} - {{ date('H:i', strtotime($end)) }} น.
                        @else
                            {{ getDateFormat($start, 11) }} - {{ getDateFormat($end, 11) }}
                        @endif
                        )
                    </span>
                @endif
        ...
        <div class="order-step-wrap mb-4">
            <ul>
                ...
                @if ($orderShopData->shipping_method != 2)
                    <li @if ($orderShopData->order_status > 2) @if ($orderShopData->order_status == 5)class="active" @else class="completed" @endif
                        @endif>
                    ...
                    @if (count($orderItems))
                        @foreach ($orderItems as $key => $item)
                            @php
                                ...
                                $prd_url = action('ProductDetailController@display', [
                                    $detail_json['cat_url'] ?? '',
                                    $item->sku,
                                ]);
                            @endphp
                            <ul>
                                <li class="product">
                                    <a href="{{ $prd_url }}"><span class="prod-img"><img
                                                src="{{ getProductImageUrlRunTime($detail_json['thumbnail_image'], 'thumb') }}"
                                                width="80" height="80" alt=""></span>
                                        <span
                                            class="prod-name">{{ $detail_json['name'][session('default_lang')] ?? $item->category_name }}</span></a>
                                </li>
                                <li><span class="mr"><img
                                            src="{{ getBadgeImageUrl($detail_json['badge']['icon'] ?? '') }}"
                                            width="30"></span></li>
                                <li> {{ $item->quantity }}
                                    {{ $detail_json['package'][session('default_lang')] ?? $item->package_name }}</li>
            ...
            <div class="box-grey col-sm-3">
                <div class="form-group">
                    <label class="radio-wrap">
                        <input type="radio" name="ord-radio" value="prepare"
                            @if ($orderShopData->seller_status == 'prepare') checked="checked" @endif>
                ...
                <div class="form-group">
                    <label class="radio-wrap">
                        <input type="radio" name="ord-radio" value="sent"
                            @if ($orderShopData->seller_status == 'sent') checked="checked" @endif>
                        <span class="radio-mark">@lang('order.sent')</span>
                    </label>
                </div>
                <div class="form-group">
                    <button class="btn- btn" id="btn_submit">ตกลง</button>
                </div>
            </div>
        @endif
    ...
    <script type="text/javascript">
        ...
        $('#btn_submit').click(function(e) {
            if ($('input[name="ord-radio"]:checked').length > 0) {
                ...
                var section =
                    status; // หรือจะ Mapping ตามที่ต้องการ เช่น status เป็น ready หรือ prepare ก็ใช้เป็น section

                var data = {
                    ord_status: status,
                    ord_id: {{ $orderShopData->id }}
                };
