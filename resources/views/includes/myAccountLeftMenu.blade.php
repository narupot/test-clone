<style>
    /* .account-nav{ background: white; } */
    /* .account-nav > h3{border-radius: 0px } */
    /* .account-nav > ul > li > a{ background: rgba(255,241,242,0.5); border: 0px;font-size: smaller;padding-left: 2rem;padding-right: 2rem} */
    /* .account-nav > ul { background: white;} */
    .account-nav li a:hover{ background: #ed1b243b; }
</style>
<div class="account-nav">
    <h3 class="bg-danger">{{ Auth::user()->display_name ?? '' }}</h3> 
    <ul class="box-grey">
        ...
        <li><a href="{{ route('user.address.index') }}" class="@if(strpos( $_SERVER['REQUEST_URI'], 'address' ) !== false) active @endif">@lang('customer.address')</a></li>
